...
async function isPropertyImage(filePath) {
  try {
    ...
    const allowedLabels = [
      ...
      'kitchen', 'bathroom', 'bedroom', 'interior','gym','washroom','condominium','Gated community'
    ];
...
router.post('/create', upload.fields([
  ...
  try {
    ...
    const validImages = [];const rejectedImages = [];
      if (req.files?.images) {
    ...
    if (req.files?.images) {
      for (const file of req.files.images) {
        ...
        if (!isValid) {
          ...
          /*updateData.
          rejectedImages.push(file.originalname);*/
          console.warn(`❌ Rejected non-property image: ${file.originalname}`);
          continue;
        }
        updateData.validPics = isValid;updateData.AIGenpics=isAIgen;
  ...
  try {
    ...
    if (req.files?.images) {
      for (const file of req.files.images) {
        ...
        const isValid = await isPropertyImage(tempPath);const isAIgen = await isAIGenerated(tempPath);
        /*if (!isValid) {
